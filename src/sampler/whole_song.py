...
class WholeSongSampler:
    ...
    def __init__(self, config):
        ...
        self.chord, self.orig = self._get_chord_and_multiprmat2c_from_config(config)
    ...
    def _get_chord_and_multiprmat2c_from_config(self, config):
        ...
        if chord.shape[0] % 32 != 0:
            ...
            multi_prmat2c = multi_prmat2c[:, :, : -(multi_prmat2c.shape[2] % 128)]
    ...
    def sample_non_ar_given_chord_and_track(self):
        ...
        for scale in self.paint_scales:
            ...
            for keep_idx in keep_indices:
                print(f"Valid tracks to keep as condition: {TRACK_NAME[keep_idx]}")

            for keep_idx in keep_indices:
                inst_folder = os.path.join(folder, f"given_{TRACK_NAME[keep_idx]}")
