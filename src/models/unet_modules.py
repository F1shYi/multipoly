...
class InterTrackAttention(nn.Module):
    def __init__(
        ...
        self.track_embedding = nn.Parameter(
            torch.randn((1, track_num, d_intertrack_encoder)), requires_grad=True
        )
    ...
    def forward(self, input_tensor):
        ...
        output_tensor = input_tensor + self.attention(
            input_tensor + self.track_embedding
        )
...
class MultitrackSequential(nn.Sequential):
    """
    ### Sequential block for modules with different inputs

    This sequential module can compose of different modules suck as `ResBlock`,
    `nn.Conv` , `SpatialTransformer` and `InterTrackAttention` and calls them with the matching signatures
    """
