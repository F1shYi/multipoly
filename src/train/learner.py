...
def get_lr_lambda(warmup_steps, decay_factor, decay_interval):
    def lr_lambda(step):
        ...
        # Step decay
        return decay_factor ** ((step - warmup_steps) // decay_interval)
    return lr_lambda
...
class Learner:
    def __init__(self, config):
        ...
        for seg_idx, seg in enumerate(self.val_segs):
            ...
            os.makedirs(os.path.join(
                self.val_dir, f"seg_{seg_idx}"), exist_ok=True)
            chord_fpath = os.path.join(
                self.val_dir, f"seg_{seg_idx}", f"val_{seg_idx}_chord.mid")
            multi_fpath = os.path.join(
                self.val_dir, f"seg_{seg_idx}", f"val_{seg_idx}_multi.mid")
            ...
            for track_idx, track in enumerate(prmat):
                midi_fpath = os.path.join(
                    self.val_dir, f"seg_{seg_idx}", f"val_{seg_idx}_track_{NAME[track_idx]}.mid")
                prmat2c_to_midi_file(track, midi_fpath)

        self.optimizer = optim.Adam(
            self.diffusion.parameters(), lr=config["training"]["lr"])
        ...
        self.generate_chord_conditioned_samples_interval = config[
            "training"]["generate_chord_conditioned_samples_interval"]
        self.generate_track_conditioned_samples_interval = config[
            "training"]["generate_track_conditioned_samples_interval"]
            ...
            for batch in tqdm(self.train_dl):
                ...
                if (self.step + 1) % self.accumulation_steps == 0:
                    ...
                    self.writer.add_scalar(
                        "Learning Rate", current_lr, self.step)

                if (self.step + 1) % self.log_train_loss_interval == 0:
                    self.writer.add_scalar(
                        'Training Loss', np.mean(running_loss), self.step)
                    running_loss = []
                    self.writer.flush()
    ...
    def generate_chord_conditioned_samples(self):
        ...
        for seg_idx, seg in enumerate(self.val_segs):
            ...
            save_folder = os.path.join(
                self.val_dir, f"seg_{seg_idx}", f"step_{self.step}")
            os.makedirs(save_folder, exist_ok=True)
            multi_prmat = torch.from_numpy(multi_prmat).to(
                self.device).reshape(1, *multi_prmat.shape)
            chord = torch.from_numpy(chord).to(
                self.device).reshape(1, *chord.shape)
                ...
                for keep_idx in [0, 1, 2, 3]:
                    for uncond_scale in [0.0, 0.5, 1.0, 2.0, 5.0]:
                        ...
                        gen = self.diffusion.paint(
                            ...
                            uncond_scale=uncond_scale,
                            uncond_cond=-
                            (torch.ones([1, 1, 512])).to(self.device)
                        )
                        gen = gen.detach().cpu().numpy()[0]
    ...
    def load_checkpoint(self, checkpoint_path):
        ...
        print(
            f"Checkpoint loaded from {checkpoint_path}, resuming from epoch {self.epoch} and step {self.step}")
